...
加えて、販売履歴CSVと外部要因CSVを使った `需要予測` と、OpenAI API を使った `チャット質問` も引き続き使えます。
...
アプリはカテゴリ単位の軽量な線形回帰で翌日需要を作り、発注計算ではその予測値を優先します。
...
そのため、優先度付き発注一覧、見送り理由、予測、チャットを同じデータから見られる構成にしています。
